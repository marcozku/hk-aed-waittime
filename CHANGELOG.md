...
**功能**：自動檢測並提示用戶授權真實地理位置，不再使用默認位置

**新增功能**：
- ✅ **自動檢測默認位置** - 系統會追踪是否使用默認位置（香港天文台）
- ✅ **醒目提示橫幅** - 使用默認位置時顯示漂亮的授權提示
- ✅ **一鍵授權** - 點擊"授權位置"按鈕立即請求真實位置
- ✅ **即時更新** - 授權成功後立即重新計算距離並排序
- ✅ **友好錯誤處理** - 清楚顯示為什麼無法獲取位置

### 🎨 提示橫幅設計
- 📍 **漸變紫色背景** - 醒目但不突兀
- ✅ **成功綠色反饋** - 授權成功後變綠色並自動消失
- ⚠️ **錯誤紅色提示** - 失敗時清楚說明原因
- 🎭 **流暢動畫** - 滑入/滑出動畫效果
...
```
1. 頁面載入 → 檢測到使用默認位置
2. 顯示提示：「使用真實位置以獲得更準確的距離」
3. 用戶點擊「授權位置」
4. 成功 → 顯示「已啟用真實位置！距離計算更準確了」
5. 自動重新排序醫院列表（按真實距離）
```
...
```javascript
// 現在清楚顯示位置類型
✅ 真實位置 (有效期剩餘: 23小時): {lat: 22.xxxx, lng: 114.xxxx}
⚠️ 默認位置 (有效期剩餘: 24小時): {lat: 22.3019, lng: 114.1742}
```

### 💾 LocalStorage 狀態追踪
新增 `isDefaultLocation` 標記：
- `true` - 使用默認位置（香港天文台）
- 不存在 - 使用真實用戶位置
